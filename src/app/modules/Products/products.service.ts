...
const createProductIntoDB = async (payload: TProduct, userData: JwtPayload) => {
  ...
  try {
    // ✅ Check user
    const user = await User.findOne({ userId: userData.userId }).session(
      session,
    );
    if (!user) {
    ...
    // ✅ Check category
    const category = await ProductCategory.findById(payload.categoryId).session(
      session,
    );
    if (!category) {
    ...
    // ✅ Auto status logic
    const status = payload.stockQuantity <= 0 ? 'OUT_OF_STOCK' : 'ACTIVE';
...
const getAllProductsFromDB = async (query: Record<string, unknown>) => {
  ...
  const data = await productCategoryQuery.modelQuery
    .populate('categoryId')
    .exec(); // <-- important to execute the query here
  return {
    meta,
    data,
  };
...
const updateProductIntoDB = async (id: string, payload: Partial<TProduct>) => {
  ...
  if (payload.stockQuantity !== undefined) {
    payload.status = payload.stockQuantity <= 0 ? 'OUT_OF_STOCK' : 'ACTIVE';
  }
