...
import { searchableFields } from "./productCategory.const";
 ...
 const createProductCategory = async (payload: TProductCategory,   userData: JwtPayload) => {
   ...
   try {
     ...
     const existingUser = await User.findOne({ userId: userData.userId }).session(session);
      if (!existingUser) {
        throw new AppError(
  ...
  // ✅ Get All Categories
const getAllProductCategoriesFromDB = async (query: Record<string, unknown>) => {
 ...
 const updateCategoryIntoDB = async (
   ...
   try {
     ...
     const existingUser = await User.findOne({ userId: userInfo.userId }).session(session);
      if (!existingUser) {
        throw new AppError(
          StatusCodes.UNAUTHORIZED,
          "User not found"
